...
Status: **IMPLEMENTED / STUDIONET LIFECYCLE COMPLETE**. The active contract finalized an official-evidence review,
credited and withdrew exactly 2 GEN, and closed with zero locked/credited accounting. Public hosting is live; Portal
...
- The full lifecycle is proven with authorized script signers. The live browser read path is proven, but a real
  browser-wallet write/sign/finality run has not yet been captured.
...
- Two superseded diagnostic testnet revisions remain intentionally marked `ABANDONED_TESTNET` at the user's direction.
  ...
  recovered. This is disclosed rather than represented as zero-accounting recovery.
