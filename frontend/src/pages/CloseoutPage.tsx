...
export function CloseoutPage() {
  ...
  return <div className="page-layout">
    ...
    <section className="action-panel" aria-labelledby="closeout-action"><p className="eyebrow">Role and state gated</p><h2 id="closeout-action">Available action</h2>
      ...
      {missing && agreement.state === "CLOSED" && sponsor ? <form onSubmit={(event) => { event.preventDefault(); transact("Fund completion closeout", () => adapter.openCloseout({ agreementId, lotId, completionStandard: standard, ratificationDeadline: new Date(ratifyDeadline).toISOString(), reviewDeadline: new Date(reviewDeadline).toISOString(), negotiationWindowSeconds: 3600 })); }}><label>TED lot ID<input value={lotId} onChange={(event) => setLotId(event.target.value)} required /></label><label>Completion release standard<textarea value={standard} onChange={(event) => setStandard(event.target.value)} minLength={20} required /></label><label>Ratification deadline<input type="datetime-local" value={ratifyDeadline} onChange={(event) => setRatifyDeadline(event.target.value)} required /></label><label>Review deadline<input type="datetime-local" value={reviewDeadline} onChange={(event) => setReviewDeadline(event.target.value)} required /></label><p className="field-help">This transaction locks exactly 1 GEN. TED amounts never set the GEN allocation.</p><button className="button button-accent" disabled={!canWrite} type="submit">Fund 1 GEN closeout</button></form> : null}
